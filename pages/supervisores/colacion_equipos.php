...
if ($id_est_equipo == 6) {
    $sql_query = "SELECT TIME_TO_SEC(hora_inicio)as hora_inicio,IF(TIME_TO_SEC(hora_ini_col)=0,1,TIME_TO_SEC(hora_ini_col)) as hora_ini_col FROM salida_equipos WHERE id_equipo =" . $id_equipo . " AND fecha = '" . $fecha . "';";
    ...
    if ($h_ini == "") {
        ...
        if ($h_ini_col != 1) {
            ...
            if ($seg_totales > $h_ini2) {
                $sql_query = "UPDATE salida_equipos set id_estado_diario = 6, hora_ini_col ='" . $hora_inicio . "' WHERE id_equipo = " . $id_equipo . " and fecha ='" . $fecha . "';";
                $result = mysqli_query($con, $sql_query);
                echo 3;
            } else {
                echo 4;
...
} else if ($id_est_equipo == 7) {
    $sql_query = "SELECT TIME_TO_SEC(hora_inicio)as hora_inicio,IF(TIME_TO_SEC(hora_ini_col)=0,1,TIME_TO_SEC(hora_ini_col)) as hora_ini_col,IF(TIME_TO_SEC(hora_fin_col)=0,1,TIME_TO_SEC(hora_fin_col)) as hora_fin_col FROM salida_equipos WHERE id_equipo =" . $id_equipo . " AND fecha = '" . $fecha . "';";
    $result = mysqli_query($con, $sql_query);
    $row = mysqli_fetch_array($result);
    ...
    if ($h_ini == "") {
        ...
        if ($h_ini_col2 != 1) {
            if ($h_fin_col2 != 1) {
                ...
                if ($seg_totales > $h_ini_col2) {
                    $sql_query = "UPDATE salida_equipos set id_estado_diario = 7, hora_fin_col ='" . $hora_inicio . "' WHERE id_equipo = " . $id_equipo . " and fecha ='" . $fecha . "';";
                    $result = mysqli_query($con, $sql_query);
                    echo 8;
                } else {
                    echo 7;
