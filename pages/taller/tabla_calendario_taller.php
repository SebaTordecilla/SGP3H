<tbody>
    ...
    for ($i = 1; $i <= $max; $i++) {
        ...
        try {
            ...
            $sql = "SELECT DISTINCT le.id_equipo as num ,le.sigla as ID, UPPER(le.nombre) as modelo,te.nombre as tipo, DATE_FORMAT((SELECT fecha FROM mant_equipos where id_equipo = " . $i . " order by fecha desc limit 1),'%d-%m-%Y') as fecha,ee.nombre as estado, IF(le.id_est_equipo=1,(SELECT SEC_TO_TIME(SUM(TIME_TO_SEC(hora_total))) FROM salida_equipos where id_equipo = " . $i . " and fecha BETWEEN '" . $ultfecha . "' and CURDATE()),'') as proxman FROM lista_equipos le LEFT JOIN tipos_equipos te on le.id_tequipo = te.id_tequipo LEFT JOIN mant_equipos me on le.id_equipo = me.id_equipo INNER JOIN estado_equipos ee on ee.id_est_equipo = le.id_est_equipo WHERE le.id_equipo = " . $i . ";";
