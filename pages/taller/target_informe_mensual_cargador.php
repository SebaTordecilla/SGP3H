...
where le.id_tequipo = 2 and le.id_est_equipo = 1 AND month(se.fecha)= " . $mes . " and year(se.fecha)= " . $ano . ") as segundos, (SELECT DAYOFMONTH(LAST_DAY('" . $ano . "-" . $mes . "-01'))) as dias,count(le.id_tequipo) as total 
...
$sql11 = "SELECT count(se.id_sal_equipo) as cantidad,(select porcentaje from programa_mensual where mes = " . $mes . " and ano = " . $ano . ") as programa FROM salida_equipos se inner join lista_equipos le on se.id_equipo = le.id_equipo where le.id_tequipo = 2 and month(se.fecha) = " . $mes . " and year(se.fecha) = " . $ano . ";";
