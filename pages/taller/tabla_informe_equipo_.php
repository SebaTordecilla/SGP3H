...
if ($ano_informe == "") {
    ...
    foreach ($db->query($maxsql) as $row) {
        ...
        FROM salida_equipos se INNER JOIN lista_equipos le on se.id_equipo = le.id_equipo INNER JOIN tipos_equipos te on le.id_tequipo = te.id_tequipo INNER JOIN reparacion_terreno rt on rt.id_sal_equipo = se.id_sal_equipo WHERE se.id_estado_diario = 5 and se.id_sal_equipo = " . $valores . " ORDER by se.fecha DESC;";
