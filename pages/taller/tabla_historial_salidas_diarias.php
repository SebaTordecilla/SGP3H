<tbody>
    ...
    for ($i = $min; $i <= $max; $i++) {
        try {
            $sql = "SELECT DATE_FORMAT(se.fecha,'%d-%m-%Y') AS fecha ,le.sigla,te.nombre,se.hora_inicio as In_Jor,se.hora_fin as Fin_Jor,
            SEC_TO_TIME(TIMESTAMPDIFF(SECOND, hora_inicio, hora_fin)) as hrs_total,
            SEC_TO_TIME(TIMESTAMPDIFF(SECOND, hora_ini_col, hora_fin_col)) hrs_col,
            IF(SEC_TO_TIME(TIME_TO_SEC(TIMEDIFF(rt.hora_mec, rt.hora_ini))+time_to_sec(rt.duraccion)) is NULL,' ',SEC_TO_TIME(TIME_TO_SEC(TIMEDIFF(rt.hora_mec, rt.hora_ini))+time_to_sec(rt.duraccion))) as hor_mec,
            se.hora_total as final
